...
* dans les cas où les objets linéaires les délimitent, les objets surfaciques sont "étendus" (se référer au schéma n°2 ci-dessous),
* dans les cas où les objets linéaires les traversent, les objets linaires se surimposent aux objets surfaciques sans les découper (se référer au schéma n°2 ci-dessous).
...
|Loisirs isolé|POINT|Tout équipement de loisirs indépendant d'un espace de loisirs intégré, identifiable dans un site cohérent (parc, square ...). L'objet restitué doit correspondre au centre de celui-ci. Si cet équipement est délimité dans une zone minérale (sable, enrobé ...), cet espace est également saisi dans la classe d'objet "Espace de loisirs" comme "autre espace de loisirs".|jeux, élément de parcours sportif, table de pique-nique ...|![picto](saisie_equi_ponctuel.png)|
